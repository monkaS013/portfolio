...
A company's staff kept asking HR the same things — benefits, policies, FAQs — spread across documents nobody wanted to dig through. Answers were slow and inconsistent. I wanted an assistant that replies in plain language and shows where the answer came from, without ever indexing salaries, performance reviews, or any personal data.

## The architecture

A sidecar pattern: the HR app (Next.js) hands the retrieval work to an isolated Python service over an internal, token-authenticated HTTP call. The retrieval logic is a LangGraph state machine:

```
retrieve → grade → route → generate | fallback
```

- **retrieve** — embed the question locally, pull the top-k chunks by cosine similarity from pgvector.
- **grade** — the model returns which chunks are actually relevant plus a confidence score (structured output, JSON schema).
- **route** — enough confidence and kept chunks → generate; otherwise → fallback.
- **generate** — answer only from the retrieved context, citing the document by name. No grounded citation → it becomes a fallback.
- **fallback** — "I couldn't find that." It never makes something up.
...
O pessoal da empresa vivia perguntando as mesmas coisas ao RH — benefícios, políticas, FAQ — espalhadas em documentos que ninguém queria vasculhar. As respostas saíam lentas e desencontradas. Eu queria um assistente que respondesse em linguagem simples e mostrasse de onde tirou a resposta, sem nunca indexar salário, avaliação de desempenho ou qualquer dado pessoal.

## A arquitetura

Padrão sidecar: o app de RH (Next.js) delega a recuperação a um serviço Python isolado, chamado por HTTP interno autenticado por token. A lógica de recuperação é uma máquina de estados em LangGraph:
...
- **grade** — o modelo devolve quais trechos são de fato relevantes mais uma confiança (structured output, JSON schema).
- **route** — confiança suficiente e trechos mantidos → generate; senão → fallback.
- **generate** — responde só pelo contexto recuperado, citando o documento pelo nome. Sem citação fundamentada → vira fallback.
- **fallback** — "Não encontrei essa informação." Nunca inventa.
