...
Management KPIs lived in three places at once: the ERP, spreadsheets on SharePoint, and desktop-only BI files. Finance, foreign trade, HR, and the operations areas each kept their own reports, in their own format, on their own cadence. Leadership had no single place to read the health of the operation or track targets against actuals. Worse, the existing reports were static and depended on one person running the refresh on their own machine. When that machine was off, the numbers froze.
...
- **Render.** The script normalizes the data and writes a single self-contained HTML file — charts as inline SVG (print-safe, no JS) or embedded Chart.js when it needs to be interactive.
...
Four dashboards in production plus a top-level panel that aggregates KPIs from up to eight areas onto one screen. Daily automatic cadence with on-demand refresh. The first consolidated view of the operation for leadership, replacing manual processes tied to one machine.
...
Os indicadores de gestão viviam em três lugares ao mesmo tempo: o ERP, planilhas no SharePoint e arquivos de BI que só abriam no desktop. Financeiro, comércio exterior, RH e as áreas de operação mantinham cada um seu relatório, no seu formato, na sua cadência. A liderança não tinha um lugar único para ler a saúde da operação nem acompanhar meta contra realizado. E os relatórios existentes eram estáticos e dependiam de uma pessoa rodar a atualização na própria máquina. Máquina desligada, dado congelado.
...
- **Geração.** O script normaliza os dados e escreve um único HTML autocontido — gráficos em SVG inline (seguro para impressão, sem JS) ou Chart.js embutido quando precisa ser interativo.
...
Quatro dashboards em produção mais um painel de topo que junta KPIs de até oito áreas numa tela só. Cadência diária automática com refresh sob demanda. A primeira visão consolidada da operação para a liderança, no lugar de processos manuais presos a uma máquina.
