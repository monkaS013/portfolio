...
A competitive-intelligence platform that measures market share per brand from public data, cross-checked against internal sell-in. It was built to replace a paid comex tool that nobody wanted to buy.
...
The company needed to know how much each competing brand sells and where its own position was gaining or losing ground, without paying for a comex tool. One fact shaped the whole design. Import data by company is not public in Brazil. So instead of "who imported," I measured what was actually installed, by manufacturer, from open regulatory data, and cross-checked it with internal sell-in.

Methodological rigor mattered as much as the numbers. Public sources publish with a lag and at different granularities, and a naive comparison flipped the sign of the conclusion more than once.

## The architecture

**Sources.** Open regulatory data on distributed generation, in two public Parquet files joined on the project key: one technical, with manufacturer and model, one registry-level, with region and tariff group. Aggregate import data by tariff code for segments the first source doesn't cover by brand. A manufacturer certification catalog. And internal sell-out/sell-in sheets, read live from SharePoint through Microsoft Graph and never versioned in the repo.

**ETL.** `fetch` downloads only when the local file is missing or stale, validates the schema by reading just the Parquet metadata (not the data), and swaps the file atomically, keeping the last good copy if a download comes back corrupt. `normalize` turns free-text manufacturer names (`197 - GROWATT`, `SOFAR SOLAR`, `NÃO HÁ`) into canonical brands, matching on word boundaries and never on raw substrings, so `SWEGON` doesn't collapse into `WEG`, and handling rebadged products confirmed by model-string evidence.
...
- **4.5M rows in a ~132 MB container.** What makes it fit is DuckDB aggregating the Parquet in SQL before Python touches the data. A naive in-container refresh would have been OOM-killed, and that constraint shaped the design.
- **Open data in place of paid import intelligence.** Reconstructing per-competitor share from what was installed, with import and certification data covering what the main source can't see.
- **Brand matching without false positives.** Free-text normalization on word boundaries, rebadge detection by model-string evidence, and model → product-family matching (~80%) against the internal catalog.
...
- **Adversarial self-audit.** 54 findings verified one by one, sorted into label/UI, bug, methodology and source limit, with explicit refutation of the ones that didn't hold up.
...
On the business side, and without figures, the platform showed that an earlier reading had understated the brand's position by mixing time windows; once the window was fixed, the competitive position in the commercial segment was much stronger than leadership had seen.
...
Uma plataforma de inteligência competitiva que mede share por marca a partir de dados públicos, cruzando com o sell-in interno. Foi feita para substituir uma ferramenta paga de comex que ninguém queria contratar.
...
A empresa precisava saber quanto cada marca concorrente vende e onde a própria posição ganhava ou perdia espaço, sem pagar por uma ferramenta de comex. Um fato moldou todo o desenho. Dado de importação por empresa não é público no Brasil. Então, em vez de "quem importou", medi o que foi de fato instalado, por fabricante, a partir de dados abertos regulatórios, e cruzei com o sell-in interno.

O rigor metodológico pesou tanto quanto os números. As fontes públicas publicam com defasagem e em granularidades diferentes, e uma comparação ingênua inverteu o sinal da conclusão mais de uma vez.

## A arquitetura

**Fontes.** Dados abertos regulatórios de geração distribuída, em dois arquivos Parquet públicos casados pela chave do empreendimento: um técnico, com fabricante e modelo, um cadastral, com região e grupo tarifário. Dado agregado de importação por código tarifário para os segmentos que a primeira fonte não cobre por marca. Um catálogo de certificação de fabricantes. E planilhas internas de sell-out/sell-in, lidas ao vivo do SharePoint via Microsoft Graph e nunca versionadas no repo.

**ETL.** O `fetch` baixa só quando o arquivo local está ausente ou velho, valida o schema lendo apenas os metadados do Parquet (não os dados) e troca o arquivo de forma atômica, preservando a última cópia boa se o download vier corrompido. O `normalize` transforma o nome de fabricante em texto livre (`197 - GROWATT`, `SOFAR SOLAR`, `NÃO HÁ`) em marca canônica, casando por fronteira de palavra e nunca por substring crua, para que `SWEGON` não vire `WEG`, e tratando rebadge confirmado por evidência de string de modelo.
...
- **4,5M de linhas num container de ~132 MB.** O que faz caber é o DuckDB agregando o Parquet em SQL antes de o Python tocar no dado. Um refresh ingênuo dentro do container teria sido OOM-killed, e essa restrição moldou o desenho.
...
- **Casamento de marca sem falso positivo.** Normalização de texto livre por fronteira de palavra, detecção de rebadge por evidência de string de modelo, e casamento modelo → família de produto (~80%) contra o catálogo interno.
...
- **Auditoria adversarial do próprio painel.** 54 achados verificados um a um, separados em rótulo/UI, bug, metodologia e limite de fonte, com refutação explícita dos que não se sustentaram.
...
No lado de negócio, e sem cifras, a plataforma mostrou que uma leitura anterior subestimava a posição da marca por misturar janelas temporais; corrigida a janela, a posição competitiva no segmento comercial era bem mais forte do que a diretoria enxergava.
